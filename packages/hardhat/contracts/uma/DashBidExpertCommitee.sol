 ...
 */
contract DashBidExpertCommitee is BaseEscalationManager, Ownable {
  ...
  function configureEscalationManager(
    ...
    emit EscalationManagerConfigured(_blockByAssertingCaller, _blockByAsserter, _validateDisputers, _arbitrateViaEscalationManager, _discardOracle);
  }
   ...
   */
  function setArbitrationResolution(bytes32 identifier, uint256 time, bytes memory ancillaryData, bool arbitrationResolution) public onlyOwner {
    bytes32 requestId = getRequestId(identifier, time, ancillaryData);
  ...
  function _checkIfAssertionBlocked(bytes32 assertionId) internal view returns (bool) {
    ...
    return
      (blockByAssertingCaller && !whitelistedAssertingCallers[assertion.escalationManagerSettings.assertingCaller]) ||
