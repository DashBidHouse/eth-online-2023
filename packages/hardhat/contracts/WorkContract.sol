...
contract WorkContract {
    ...
    event WorkContractPayoutRequested(
        bytes32 indexed workContractId,
        bytes32 indexed assertionId
    );

    event WorkContractPayoutSettled(
        bytes32 indexed workContractId,
        bytes32 indexed assertionId
    );
    ...
    function issueWorkContract(
        ...
        require(
            workContracts[workContractId].payoutAddress == address(0),
            "WorkContract already exists"
        );
    ...
    function _settlePayout(bytes32 assertionId) internal {
        ...
        defaultCurrency.safeTransfer(
            workContract.payoutAddress,
            workContract.paymentAmount
        );
